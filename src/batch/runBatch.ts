...
/**
 * =====================================================================
 * Helpers (汎用 / Notion変換 / HTTP など)
 *  - ここは「処理の骨格」ではなく支援関数群
 * =====================================================================
 */

type NotionWriteRichText = {
	type: 'text';
	text: { content: string };
};
 ...
 */
const toNotionRichText = (input: string, limit = 1990, maxChunks = 100): NotionWriteRichText[] => {
	if (!input) {
		return [];
	}
	...
	while (rest.length > 0 && chunks.length < maxChunks) {
		...
		const cutIndex = lastNewline > Math.floor(limit * 0.6) ? lastNewline + 1 : limit;
