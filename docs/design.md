...
### フィードバック整形ルール（Markdown → Notion blocks）

- Gemini が返した Markdown を Notion ブロックへ変換して書き戻す
- 見出し（`#`〜`###`）・段落・箇条書き（`- item`）・太字（`**bold**`）に対応
- **箇条書きはネストせず、すべてトップレベルのブロックとしてフラット化する**
  - インデント付き箇条書きの階層は、本文先頭の全角スペース（インデント段数ぶん）で表現する
  - 理由：Notion の append-children API は深いネスト（`children`）を受け付けず、
    `400 validation_error` になるため。Gemini 出力の階層深度に依存せず確実に書き戻せるよう、
    境界（Markdown → Notion）で正規化する
- 未対応の記法（テーブル・コードブロック・番号付きリスト等）は段落テキストへフォールバックする
